...
​```typescript
// ESM
import { createWiki, Wiki, WikiOptions } from 'deepwiki';

// CommonJS
const { createWiki, Wiki, WikiOptions } = require('deepwiki');
​```
...
这是 deepwiki 的主要入口函数，用于初始化一个新的 Wiki 生成器实例。
...
​```typescript
import { createWiki } from 'deepwiki';
...
​```typescript
import { createWiki } from 'deepwiki';
import { apiDocEnhancer, diagramPlus } from 'deepwiki/plugins';
...
​```typescript
import { createWiki, ValidationError, FileSystemError } from 'deepwiki';
