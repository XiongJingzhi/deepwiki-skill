...
def test_finalize_dispatches_quality(monkeypatch, tmp_path):
    ...
    assert code == 1
    cmd = calls[0]
    assert cmd[1].endswith("check_doc_quality.py")
    assert cmd[2:] == [str(tmp_path), "--verbose", "--json", "quality.json"]
...
def test_finalize_dispatches_consistency(monkeypatch, tmp_path):
    ...
    assert code == 0
    cmd = calls[0]
    assert cmd[1].endswith("check_cross_module_consistency.py")
    assert cmd[2:] == [str(tmp_path), "--json", "consistency.json"]
