...
Generate **professional-grade** structured project Wiki to `.deepwiki/` directory.
...
- Write wiki files to `.deepwiki/wiki/`
...
```bash
# 运行动态质量检查
python scripts/check_quality.py .deepwiki --analyze-complexity
```
...
- **GitHub**: `owner/repo` (e.g., `trsoliu/deepwiki-extras`)
...
| Script | Usage |
|--------|-------|
| `scripts/init_wiki.py <path>` | Initialize .deepwiki directory |
