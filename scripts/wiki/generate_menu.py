...
try:
    from jsonschema import validate as _js_validate, ValidationError as _js_ValidationError
except ImportError:
    _js_validate = None
    _js_ValidationError = None
...
def _build_menu_from_topology(
    ...
    for raw_group_title, page_ids_or_subgroups in groupings.items():
        section_title = group_label_map.get(raw_group_title, raw_group_title)
        direct_items: List[Dict[str, Any]] = []

        if isinstance(page_ids_or_subgroups, dict):
            # Nested sub-groupings → 3-level menu
            for sub_title, sub_page_ids in page_ids_or_subgroups.items():
                if not isinstance(sub_page_ids, list):
                    continue
                sub_items: List[Dict[str, str]] = []
                for page_id in sub_page_ids:
        ...
        elif isinstance(page_ids_or_subgroups, list):
            # Flat list → 2-level menu
            for page_id in page_ids_or_subgroups:
                page = page_map.get(page_id)
                if not page:
                    continue
    ...
    # 去重：移除叶子路径完全被其他组覆盖的冗余分组
    # （例如 doc-topology.json 同时有语义分组和兜底 "deep-dive" 分组时）
    def _leaf_paths(group: Dict) -> set:
        paths: set = set()
        def _walk(items):
            for item in items:
                if item.get('path'):
        ...
        _walk(group.get('items', []))
        return paths

    if len(menu) > 1:
        groups_paths = [_leaf_paths(g) for g in menu]
        _BUILTIN_TITLES = {'概览', '理解项目', '深入理解', '参考资料', '更多'}
        to_remove: set = set()
        for i in range(len(menu)):
            # 仅考虑移除内置分组（兜底 deep-dive / concepts 等）；
            # 语义分组即使路径被覆盖也保留，因为它提供更有价值的导航结构。
            if menu[i].get('title', '') not in _BUILTIN_TITLES:
                continue
            other_paths = set().union(
                *(p for j, p in enumerate(groups_paths) if j != i)
            )
            if groups_paths[i] and not (groups_paths[i] - other_paths):
                to_remove.add(i)
        menu = [g for i, g in enumerate(menu) if i not in to_remove]
...
def reconcile_menu(wiki_dir: str, project_name: str = '',
    ...
    for group in menu_data.get('items', []):
    ...
    def _collect_paths(items: List[Dict]) -> set:
        paths: set = set()
        for item in items:
            if item.get('path'):
                paths.add(item['path'])
            paths.update(_collect_paths(item.get('items', [])))
        return paths

    known_paths: set = set()
    for group in updated_menu:
        known_paths.update(_collect_paths(group.get('items', [])))
    ...
    # Re-collect known_paths after _KNOWLEDGE_DIRS scan may have added items
    known_paths = set()
    for group in updated_menu:
        known_paths.update(_collect_paths(group.get('items', [])))

    # --- Full wiki scan: find ANY .md file not yet in menu ---
    all_wiki_files: set = set()
    for md_file in wiki_path.rglob('*.md'):
        rel = md_file.relative_to(wiki_path).as_posix()
        all_wiki_files.add(rel)

    orphan_files = all_wiki_files - known_paths
    if orphan_files:
        # Classify orphans by directory into sections
        dir_section_map = {
            'concepts': L.get('concepts', 'concepts'),
            'deep-dive': L.get('deep-dive', 'deep-dive'),
            'reference': L.get('reference', 'reference'),
        }
        orphans_by_dir: Dict[str, List[Dict[str, str]]] = {}
        for rel_path in sorted(orphan_files):
            parts = rel_path.split('/')
            dir_name = parts[0] if len(parts) > 1 else ''
            section_key = dir_section_map.get(dir_name, '__other__')
            orphans_by_dir.setdefault(section_key, [])
            title = extract_title(str(wiki_path / rel_path))
            orphans_by_dir[section_key].append({
                'title': title,
                'path': rel_path,
            })

        for section_title, orphan_items in orphans_by_dir.items():
            if section_title == '__other__':
                section_title = L.get('other', '其他')
            existing = next((g for g in updated_menu if g.get('title') == section_title), None)
            if existing:
                existing.setdefault('items', []).extend(orphan_items)
            else:
                updated_menu.append({'title': section_title, 'items': orphan_items})
        changes['added'] += len(orphan_files)
        if verbose:
            print(f"  新增孤儿文件: {len(orphan_files)} 个")
            for rel_path in sorted(orphan_files):
                print(f"    + {rel_path}")

    final_menu = [g for g in updated_menu if g.get('items')]

    # Deduplicate items by path within each group
    for group in final_menu:
        seen: set = set()
        ...
        for item in group.get('items', []):
            ...
            if path:
                seen.add(path)
            # Also deduplicate nested items
            if item.get('items'):
                nested_seen: set = set()
                nested_deduped: List[Dict[str, Any]] = []
                for sub in item['items']:
                    sp = sub.get('path', '')
                    if sp and sp in nested_seen:
                        continue
                    if sp:
                        nested_seen.add(sp)
                    nested_deduped.append(sub)
                item['items'] = nested_deduped
            deduped.append(item)
        group['items'] = deduped

    menu_data['items'] = final_menu
    menu_data['reconciled'] = True
    menu_data['reconciled_at'] = datetime.now(timezone.utc).isoformat()
    menu_data.pop('planned', None)

    for key in ('titles_updated', 'planned_removed', 'missing_removed', 'added', 'semantic_group_hints'):
...
def _empty_menu(project_name: str) -> Dict[str, Any]:
    return _envelope([], project_name)


def _load_schema() -> dict:
    """加载 menu-schema.json。"""
    schema_path = Path(__file__).resolve().parent.parent.parent / 'schemas' / 'menu-schema.json'
    if not schema_path.exists():
        print(f"警告: schema 文件不存在: {schema_path}", file=sys.stderr)
        return {}
    with open(schema_path, encoding='utf-8') as f:
        return json.load(f)


def validate_menu(menu_data: Dict[str, Any]) -> List[str]:
    """校验 menu_data 是否符合 menu-schema.json，返回错误列表（空表示通过）。"""
    if _js_validate is None:
        print("警告: jsonschema 未安装，跳过 menu.json schema 校验", file=sys.stderr)
        return []
    schema = _load_schema()
    if not schema:
        return []
    errors: List[str] = []
    try:
        _js_validate(instance=menu_data, schema=schema)
    except _js_ValidationError as e:
...
def save_menu(wiki_dir: str, menu_data: Dict[str, Any]) -> str:
    ...
    errors = validate_menu(menu_data)
    if errors:
        print(f"错误: menu.json schema 校验失败:", file=sys.stderr)
        for err in errors:
...
示例:
  ...
  python generate_menu.py /path/to/.deepwiki/wiki "项目名称" --reconcile
    ...
    if args.reconcile:
        ...
        print(f"[{mode_label}] 修正完成 -> {output}")
    else:
        # 自动推断 cache_dir：wiki_dir 的父目录下的 cache/
        inferred_cache = Path(wiki_dir).parent / 'cache'
        cache_dir = str(inferred_cache) if inferred_cache.exists() else None
