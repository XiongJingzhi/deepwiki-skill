#!/usr/bin/env python3
"""Local wiki documentation server.

Reads menu.json and serves markdown wiki files as a single-page web app
with Mermaid diagram support and syntax highlighting.

Usage:
    python serve_wiki.py <project_path> [--port PORT] [--host HOST]
"""
...
def _load_menu(wiki_dir: Path) -> dict:
    menu_path = wiki_dir / "menu.json"
    if menu_path.is_file():
        with open(menu_path, "r", encoding="utf-8") as f:
            menu = json.load(f)
            if isinstance(menu, dict) and isinstance(menu.get("menu"), list):
                return menu
            return {"title": menu.get("title", "Wiki") if isinstance(menu, dict) else "Wiki", "menu": []}
    return {"title": "Wiki", "menu": []}
...
.menu-group-title:hover { color: var(--text); }
...
.menu-item a:hover { background: #e9ecf0; text-decoration: none; }
.menu-item.active a {
  background: #dbeafe;
...
.menu-sub-item a { padding-left: 44px; font-size: 13px; color: var(--text-secondary); }
.menu-sub-item.active a { color: var(--accent); }
...
function navigateToDefault() {
  for (const group of menuData.menu || []) {
...
function buildMenuItemsHTML(items, level) {
  ...
  for (const item of items) {
    if (item.path) {
      const cls = item.items && item.items.length ? "menu-item menu-sub-item" : "menu-item";
...
function setupSourceLinks(root) {
  root.querySelectorAll('a[href^="file://"]').forEach((link) => {
    ...
    const label = link.textContent.trim() || parsed.path;
    const displayLabel = label.startsWith("file://") ? parsed.path.replace(/^\/+/, "") : label;
...
class WikiHandler(BaseHTTPRequestHandler):
    ...
    def _serve_source(self, query):
        ...
        if requested.is_absolute():
            ...
            if not file_path.is_relative_to(root) and not file_path.exists():
                file_path = (root / raw_path.lstrip("/")).resolve()
        else:
...
def serve_wiki(project_path: str, port: int = 8742, host: str = "127.0.0.1"):
    global wiki_dir, project_root
    project_root = Path(project_path).resolve()
    wiki_dir = resolve_wiki_dir(project_path)
    menu = _load_menu(wiki_dir)

    server = HTTPServer((host, port), WikiHandler)
    url = f"http://{host}:{port}"
    print(f"DeepWiki server running at {url}")
    print(f"  Project: {menu.get('title', wiki_dir.parent.parent.name)}")
