...
支持子命令模式：python finalize.py <command> [options]

用法:
    python scripts/finalize.py mermaid <.deepwiki路径>
    python scripts/finalize.py quality <.deepwiki路径>
    python scripts/finalize.py consistency <.deepwiki路径>
"""
...
def main():
    parser = argparse.ArgumentParser(description="DeepWiki 文档收尾工具")
    ...
    if args.command == "mermaid":
        script = os.path.join(scripts_dir, "fix_mermaid.py")
        ...
        script = os.path.join(scripts_dir, "check_doc_quality.py")
        cmd = [sys.executable, script, args.wiki_dir]
        if args.verbose:
            cmd.append("--verbose")
        if args.json:
        ...
        script = os.path.join(scripts_dir, "check_cross_module_consistency.py")
        cmd = [sys.executable, script, args.wiki_dir]
        if args.json:
            cmd.extend(["--json", args.json])
