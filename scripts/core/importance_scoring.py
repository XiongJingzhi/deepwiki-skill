...
# ---- 阈值常量（小项目 fallback 和 scanner 初始分类使用） ----
# 大项目（>= 20 文件）由 _apply_dynamic_thresholds 基于百分位动态覆盖
CORE_IMPORTANCE_THRESHOLD = 0.5
HIGH_PRIORITY_IMPORTANCE_THRESHOLD = 0.6
...
def _apply_dynamic_thresholds(files: List[Dict[str, Any]]) -> None:
    ...
    if len(scores) < 20:
        core_threshold = CORE_IMPORTANCE_THRESHOLD
        high_priority_threshold = HIGH_PRIORITY_IMPORTANCE_THRESHOLD
    else:
