...
def _load_module_analysis(project_root: Path) -> Dict[str, Any]:
    data = load_module_analysis(project_root, validate_version=True)
    if data is None:
        raise FileNotFoundError("module-analysis.json not found or version mismatch")
...
def _evidence_for_module(module: Dict[str, Any]) -> List[Dict[str, Any]]:
    ...
    for file_data in module.get("files", []):
        ...
        if path:
            ranges = extract_file_source_ranges(file_data, include_reason=False)
            item: Dict[str, Any] = {
                "type": "file",
                "path": path,
