...
from scripts.core.common import cache_path, load_module_analysis as _load_module_analysis
...
def check_analysis_quality(
    ...
    analysis_modules = module_analysis
    if modules:
        analysis_modules = {
            k: v for k, v in module_analysis.items() if k in modules
        }

    checked_modules = list(analysis_modules.keys())

    for mod_name, data in analysis_modules.items():
    ...
    return all_errors, all_warnings, failed_modules, checked_modules
...
def main() -> int:
    ...
    module_analysis = _load_module_analysis(project_path)
