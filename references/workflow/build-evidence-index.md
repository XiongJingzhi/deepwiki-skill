# build-evidence-index：构建源码证据索引

> `build-evidence-index` 从 `module-analysis.json` 中抽取关键 claim，并把 claim 关联到可验证的源码文件证据，输出 `cache/evidence-index.json`。

## 位置

| 项 | 内容 |
|----|------|
| **前置** | `extract-docs`、`plan-doc-topology`、`check-analysis-quality` |
| **后置** | `generate-module-docs`、`finalize quality`、跨页面一致性检查 |
