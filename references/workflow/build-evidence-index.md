...
- `claims[].page_id`：该 claim 对应的页面，例如 `deep-dive:auth`
- `claims[].claim_text`：结论文本
- `claims[].evidence[]`：源码证据列表，至少包含文件路径；若模块分析提供 `core_source_ranges` 或 `public_interfaces.line/end_line`，同时包含 `ranges[].start_line/end_line/label`
