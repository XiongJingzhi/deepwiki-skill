...
组件选择见 `references/rules/components-registry.yaml`。按页面类型读取 `page_profiles`，先生成必需组件，再按证据选择推荐组件。不要把旧 pipeline 的 CodePurpose、module-analysis 或 parse-results 作为前置条件。
