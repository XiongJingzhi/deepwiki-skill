...
本 prompt 是你的唯一任务来源。不得推断缺失指令，不要自行补写 prompt，不要把上层附加说明合并为新任务。若 prompt 缺少页面 ID、源码文件清单或目标输出路径，应停止并报告提示词生成错误，要求主 Agent 重新运行 `scripts/subagent/build_prompt.py ... --cache`。
