...
| `state/progress.json` | 分阶段任务状态机（overview/menu/details 三阶段，每模块 pending/in_progress/completed/failed） |
...
| `wiki/menu.json` | 层级化导航菜单（概览 → 理解项目 → 深入理解 → 参考资料 / 更多），自动由 `generate_menu.py` 生成 |
| `wiki/concepts/` | 自上而下的理解材料：架构、设计思路、关键概念、继续开发心智模型 |
| `wiki/concepts/` | 自上而下的理解材料：架构、设计思路、关键概念、继续开发心智模型 |
| `wiki/deep-dive/` | 深入理解源码学习页；每页整合能力、核心逻辑、接口、内部机制、风险和扩展点 |
| `wiki/reference/` | 查阅型资料：接口索引、配置索引、Schema、术语表等 |
| `cache/modules/{slug}/context.json` | `prepare_module_context.py` 生成的模块预切片上下文（模块文件列表、调用图切片、导入关系切片、预填充字段）。**新增字段**：`files[].signatures`（按 `analysis_depth` 分档的函数/类签名列表，deep 级含 name/kind/line/end_line/doc/params/returns，standard 级仅含有注释签名，quick 级为空数组）、`files[].exports`（顶层导出符号名列表）。`extract-docs` 阶段禁止读取源码，直接消费此文件 |
...
| `state/checksums.json` | `detect-changes` | `detect-changes` | 否 | 是 |
| `state/progress.json` | 多阶段协作更新 | 全流程恢复与重试 | 否 | 是 |
