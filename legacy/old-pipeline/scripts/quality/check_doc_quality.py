...
def _has_top_relevant_source_files(content: str) -> bool:
    ...
    Accepts both inline or separate-line format.
    """
    return bool(
        re.search(
            r'\A\s*#\s+[^\n]+\n+'
...
class QualityMetrics:
    ...
    # 服务端可访问性检测（通过 /api/source 请求验证）
    server_link_accessible_count: int = 0   # 服务端可访问的链接数
    server_link_inaccessible_count: int = 0  # 服务端不可访问的链接数
...
def analyze_document(file_path: str, structure_path: str = None,
                    project_root: str = None, server_url: str = None) -> QualityMetrics:
    """分析单个文档的质量"""
    ...
    # 检查源码追溯：file:// 链接即可（不需要行号范围）
    source_link_count = len(re.findall(r'file:///[^)\s#]+', content))
    metrics.has_source_tracing = source_link_count > 0
    ...
    # 服务端可访问性闭环验证（可选，需要 serve_wiki.py 正在运行）
    if server_url and metrics.has_source_tracing:
        accessible, inaccessible = validate_source_links_via_server(content, server_url)
        metrics.server_link_accessible_count = accessible
        metrics.server_link_inaccessible_count = inaccessible
...
def validate_source_links_via_server(
    content: str,
    server_url: str,
    timeout: float = 3.0,
) -> Tuple[int, int]:
    """Validate file:// links by calling the local wiki server's /api/source endpoint.

    For each unique file:// link in *content*, constructs a request to
    ``{server_url}/api/source?path=<file_path>`` and checks whether the server
    responds with HTTP 200.  This acts as an end-to-end accessibility check,
    verifying that the server can resolve and serve the referenced source file
    (regardless of whether the path is absolute, relative, or uses the
    file:// URI scheme).

    Args:
        content:    Markdown document text.
        server_url: Base URL of the running DeepWiki server, e.g.
                    ``http://127.0.0.1:8742``.  Trailing slashes are stripped.
    ...
    # Extract all file:// hrefs (path only, strip fragment)
    ...
    for m in pattern.finditer(content):
        raw = m.group(0)  # full file://... string
        # Strip fragment (#L10-L20)
        raw_no_frag = raw.split("#")[0]
        if raw_no_frag in seen:
            continue
        seen.add(raw_no_frag)

        # Build query: send the raw path (server will resolve it)
        params = urllib.parse.urlencode({"path": raw_no_frag})
        url = f"{server_url}/api/source?{params}"
        try:
...
def generate_issues(m: QualityMetrics, structure_path: str = None) -> List[str]:
    ...
    if doc_type == "module":
        if not m.has_relevant_source_files:
            issues.append("模块文档标题后应立即包含「相关源文件」(Relevant source files) 折叠源码索引（使用 <details open>）")
    ...
    if m.server_link_inaccessible_count > 0:
        total_srv = m.server_link_accessible_count + m.server_link_inaccessible_count
        issues.append(
            f"服务端无法访问的源码链接: {m.server_link_inaccessible_count}/{total_srv} 个 "
            f"(通过 /api/source 验证)"
        )
...
def check_wiki_quality(wiki_path: str, server_url: str = None) -> QualityReport:
    """检查整个 Wiki 目录的质量"""
...
def print_report(report: QualityReport, verbose: bool = False):
    ...
    # 服务端可访问性汇总（仅在运行了 --server-url 检查时显示）
    total_srv_accessible = sum(d.server_link_accessible_count for d in report.docs)
    total_srv_inaccessible = sum(d.server_link_inaccessible_count for d in report.docs)
    if total_srv_accessible > 0 or total_srv_inaccessible > 0:
        print(f"| 服务端链接可访问性 | ✅ {total_srv_accessible} / ❌ {total_srv_inaccessible} |")
    print()
...
示例:
  ...
  python check_doc_quality.py . --json report.json
    ...
    parser.add_argument(
        ...
        metavar="URL",
        default=None,
        help="DeepWiki 服务器地址（如 http://127.0.0.1:8742），启用后对每个 file:// 链接向 /api/source 发请求验证可访问性"
    )
    ...
    # 执行检查
    report = check_wiki_quality(wiki_path, server_url=getattr(args, "server_url", None))
