...
from scripts.analysis.import_relations import compute_in_degree
from scripts.analysis.import_relations import extract_import_relations
from scripts.core.parsers import get_manager, get_lang_for_ext
...
def detect_archetype(project_path: Path) -> str:
    ...
    rules = [
        ...
        ("agent-project", lambda p: (
            manifest_has_dependency(p, {"langchain", "langgraph", "pi-mono", "llamaindex", "crewai", "autogen", "semantic-kernel"})
            or (p / "agents").is_dir()
        )),
        ("ml-project", lambda p: manifest_has_dependency(p, {
            "torch", "tensorflow", "keras", "scikit-learn", "sklearn",
            "transformers", "pytorch", "jax", "paddle",
        })),
        ("data-pipeline", lambda p: (
            manifest_has_dependency(p, {"airflow", "prefect", "dagster", "luigi", "pyspark",
                         "apache-beam", "kafka", "pulsar"})
            or (p / "dags").is_dir()
        )),
        ("microservice", lambda p: manifest_has_dependency(p, {
        ...
        ("serverless", lambda p: (
            ...
            or manifest_has_dependency(p, {"@aws-lambda/core", "azure-functions", "vercel"})
        )),
        ("cli-tool", lambda p: (
            manifest_has_dependency(p, {"commander", "yargs", "inquirer", "oclif", "clipanion",
                         "clap", "cobra", "click", "typer", "argparse-rs"})
            or (p / "cmd").is_dir()
            or _cargo_has_bin(p)
        )),
        ("web-service", lambda p: manifest_has_dependency(p, {
            "fastapi", "flask", "django", "express", "koa", "fastify", "hono",
            "gin", "fiber", "axum", "actix-web", "spring", "rails",
        })),
        ("desktop-app", lambda p: (
            manifest_has_dependency(p, {"electron", "tauri", "wry", "flutter", "slint",
                         "pyqt", "pyside", "tkinter", "kivy"})
        )),
        ("mobile-app", lambda p: (
            manifest_has_dependency(p, {"react-native", "expo", "capacitor", "ionic",
                         "swiftui", "jetpack-compose"})
            or (p / "android").is_dir() or (p / "ios").is_dir()
        )),
        ("spa-frontend", lambda p: (
            manifest_has_dependency(p, {"react", "vue", "svelte", "angular", "solid-js"})
        ...
        ("sdk-library", lambda p: (
            ...
            and not manifest_has_dependency(p, {"react", "vue", "express", "fastapi", "flask"})
            and has_manifest(p)
...
def _collect_parse_summaries(
    ...
    """
    from scripts.core.code_metrics import compute_complexity_score, count_loc as _count_loc, scan_todo_lines as _scan_todo
    ...
    for fpath in files:
        ...
        try:
            ...
            try:
                from scripts.pipeline.extract_doc_comments import _extract_doc_entries
