...
from scripts.core.common import IGNORE_DIRS, IGNORE_FILES, CODE_EXTENSIONS, HASH_TRUNCATE_LENGTH, should_ignore_path, calculate_file_hash
from scripts.core.importance_scoring import calculate_file_importance, CORE_IMPORTANCE_THRESHOLD, HIGH_PRIORITY_IMPORTANCE_THRESHOLD
from scripts.core.code_metrics import estimate_complexity, count_important_lines, compute_complexity_and_important_lines
...
def scan_files(root_path: Path, gitignore_cache=None, parse_cache=None,
    ...
    for f in root_path.rglob('*'):
        ...
        files.append({
            ...
            'is_core': importance >= CORE_IMPORTANCE_THRESHOLD,
            # 高优先级档位，用于关系分析和深度分析阶段的精确过滤
            # 大项目（>= 20 文件）由 _apply_dynamic_thresholds 基于百分位动态覆盖
            'is_high_priority': importance >= HIGH_PRIORITY_IMPORTANCE_THRESHOLD,
            # 原始分数（用于模块内归一化）
