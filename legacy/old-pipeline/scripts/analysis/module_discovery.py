...
from scripts.core.common import IGNORE_DIRS, CODE_EXTENSIONS, UnionFind, build_file_to_module_map
...
MAX_MODULE_PATH_DEPTH = 8
WORKSPACE_MAX_MODULE_PATH_DEPTH = 10
...
def _read_workspace_packages(root_path: Path) -> List[str]:
    ...
    优先级：
    1. pnpm-workspace.yaml / package.json workspaces（显式配置）
    2. 直接存在的 packages/ 或 apps/ 目录（隐式 monorepo，无需配置文件）
    """
    packages = []
    ...
    # Fallback: 无显式配置但存在 packages/ 或 apps/ 目录时，
    # 将其视为 monorepo workspace 根，以子包为模块划分单位。
    for implicit_dir in ('packages', 'apps'):
        if implicit_dir not in packages:
            candidate = root_path / implicit_dir
            if candidate.is_dir():
                # 目录下至少有一个含代码文件的子包才纳入
                sub_pkgs = [
...
def _can_split_directory(root_path: Path, path: Path, exclude_dirs: Set[str],
                         max_depth: int = MAX_MODULE_PATH_DEPTH) -> bool:
    return (
        _module_path_depth(root_path, path) < max_depth
...
def _split_directory_modules(
    ...
    refined_by: List[str] = None,
    max_depth: int = MAX_MODULE_PATH_DEPTH,
) -> List[Dict[str, Any]]:
    """Split natural submodules recursively, capped at max_depth."""
    if _can_split_directory(root_path, path, exclude_dirs, max_depth):
...
def discover_modules(root_path: Path, exclude_dirs: Set[str] = None,
    ...
    # monorepo workspace 包位于 packages/pkg 或 apps/pkg（深度 2-3），
    # 需要更深的拆分深度才能探入包内部子模块，而非将整个包作为一个模块。
    # （如 Java monorepo: packages/auth/src/main/java/com/company/auth/ = 深度 9）
    workspace_max_depth = WORKSPACE_MAX_MODULE_PATH_DEPTH if workspace_dirs else MAX_MODULE_PATH_DEPTH
    ...
    for src_dir in src_dirs:
        ...
        max_depth = workspace_max_depth if src_dir in workspace_dirs else MAX_MODULE_PATH_DEPTH
...
def _update_module_scores(modules: List[Dict], all_files: List[Dict]) -> None:
    """根据文件重要性重新计算模块的 importance_score、core_files 和 core_files_count。

    在 discover_modules（初始评分）和 normalize_path_scores（归一化后重算）后
    都应调用此函数，确保模块评分与当前文件分数一致。
    """
    if not all_files:
        return
    for mod in modules:
        ...
        if mod_files:
            ...
            core_files = [f['path'] for f in mod_files if f['is_core']][:20]
            mod['importance_score'] = round(avg_score, 2)
            mod['core_files'] = core_files
            mod['core_files_count'] = len(core_files)
        else:
    ...
    for src_file, targets in import_relations.items():
        ...
        for tgt_file in targets:
            if tgt_file in mod_files:
                uf.union(src_file, tgt_file)

    return [set(g) for g in uf.groups()]
...
def refine_modules(
    ...
    file_to_module = build_file_to_module_map(modules, project_root=project_root, scan_fs=True)
